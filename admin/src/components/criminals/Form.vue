...
import { UPLOAD_CRIMINALS_RECORDS } from '@/store/actions.type'
import { deepClone, onErrors, setValues } from '@/utils'

const name = 'CriminalsForm'
...
function onFileAdded(files) {
	...
	store.dispatch(UPLOAD_CRIMINALS_RECORDS, { file: files[0]})
	.then(records => {
...
<template>
	<form @submit.prevent="onSubmit">
		...
		<v-row dense v-show="has_records">
			...
			<v-col cols="12">
				<CriminalsTable :records="state.form.records" />
			</v-col>
		</v-row>
	</form>
</template>