...
import { INIT_USERS, FETCH_USERS, CREATE_USER, STORE_USER, SYNC_USERS, UPDOWN_USERS
...
const labels = computed(() => store.state.users.labels)
...
function onUpDown(active) {
	let ids = table.value.getCheckIds()
	let up = !active
	store.dispatch(UPDOWN_USERS, { ids, up })
	.then(() => {
		fetchData()
		table.value.setCheckIds([])
		onCancel()
		onSuccess()
	})
	.catch(error => handleSubmitError(error))
...
<template>
	<div>
		<UserHead ref="head" :query="query" :labels="labels" 
		:active_options="active_options"
		:roles="roles" :users="state.users"
		:departments="departments"
		:show_down="state.can_down"
		@submit="fetchData" @create="onCreate" 
		@sync="onSync" @updown="onUpDown"
		/>
		<v-row dense>
			<v-col cols="12">
				<UserTable ref="table" :labels="labels" 
