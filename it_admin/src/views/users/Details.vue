...
import { INIT_USERS, USER_DETAILS, PAGE_NOT_FOUND, FETCH_ROLES, EDIT_USER, UPDATE_USER, 
...
<template>
	<div>
		<div v-if="!isEmptyObject(state.user)">
			<v-card>
				...
				<v-card-text>
					<UserView :model="state.user" :labels="labels"
					/>
				</v-card-text>
			</v-card>
				...
				<v-window v-model="state.tab.value">
					<v-window-item :value="ENTITY_TYPES.PROFILES.name">
						...
						<v-card v-else :max-width="WIDTH.M">
							...
							/>
							<v-card-text>
								<ProfilesView :model="state.user.profiles" :labels="getLabel('profiles')"
								:roles="roles" 
								/>
							</v-card-text>
						</v-card>
		...
		<v-dialog persistent v-model="state.form.active" :width="WIDTH.M + 50">
			<v-card v-if="state.form.active" :max-width="WIDTH.M">
				...
				<v-card-text>
					...
					:model="state.form.model" :roles="roles" :active="true" :labels="labels"
					@submit="onSubmit" 
					/>
