...
import { toYearTW, toMonthTW } from '@/utils'
...
<template>
	<div class="v3dp__popout" :class="`v3dp__popout-${viewMode}`"
		:style="{ ['--popout-column-definition']: `repeat(${columnCount}, 1fr)` }" @mousedown.prevent>
		<div class="v3dp__heading">
			<button class="v3dp__heading__button v3dp__heading__button__left" :disabled="leftDisabled"
				@click.stop.prevent="$emit('left')">
				<slot name="arrow-left">
					<svg class="v3dp__heading__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 6 8">
			...
			<component :is="headingClickable ? 'button' : 'span'" class="v3dp__heading__center"
				@click.stop.prevent="$emit('heading')">
				<slot name="heading" />
			</component>
			<button class="v3dp__heading__button v3dp__heading__button__right" :disabled="rightDisabled"
		...
		<div class="v3dp__body">
			...
			<div class="v3dp__elements">
				<slot name="body">
					<button v-for="item in items" :key="item.key" :disabled="item.disabled" :class="[
						{
						...
						<span>{{ getDisplayStyle(item) }}</span>
					</button>
				</slot>
			</div>
		</div>
