...
const emit = defineEmits(['submit', 'sum_report', 'report'])
defineExpose({
...
function onSumReport() {
	emit('sum_report', state.query)
}
</script>
   ...
   <form v-show="!isEmptyObject(state.query)" @submit.prevent="onSubmit">
		<v-row dense>
         ...
         <v-col cols="3">
			...
			<v-col cols="2">
				<h4 class="pl-3 pt-3">總計： {{ total }} 人次</h4>
			</v-col>
			<v-col cols="1">
				<v-tooltip text="統計報表" v-if="can_report">
					<template v-slot:activator="{ props }">
						<v-btn class="float-right" 
						icon="mdi-file-document" v-bind="props" size="small" color="info"
						@click.prevent="onSumReport"
						/>
					</template>
				</v-tooltip>
			</v-col>
			<v-col cols="3">
				<v-tooltip text="明細報表" v-if="can_report">
					<template v-slot:activator="{ props }">
						<v-btn class="float-right" 
